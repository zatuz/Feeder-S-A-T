...
echo "segun la zona horaria que deverias manejar es $(curl -fsSL http://ip-api.com/line?fields=timezone)"
...
if [ "$TIEMPO" == "y" ]; then
    ...
    timedatectl set-timezone "$(curl -fsSL http://ip-api.com/line?fields=timezone)"
elif [ "$TIEMPO" == "n" ]; then
